...
import type { CartItem } from "./Interface.js";

const Navbar = ({ cartItems }: { cartItems: CartItem[] }) => {
    return (
        <nav className="flex justify-between fixed top-0 items-center z-50 py-5 md:px-10 px-2 w-full">
                <h1 className="bg-[#ecf39e] text-[#132a13] py-2 px-10 rounded-3xl md:text-3xl tracking-[0.2em] font-bold font-home">EPIKCART</h1>
                <div className="flex items-center mr-2 gap-5 md:gap-10">
                    <Link to='/'>
                    ...
                    <Link to='cart' className="relative inline-flex items-center">
                        ...
                        <span className="absolute -top-3 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">
                            {cartItems.length}
                        </span>
                    </Link>
                </div>
